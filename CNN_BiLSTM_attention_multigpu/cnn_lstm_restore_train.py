    ...
    with tf.Graph().as_default():
        ...
        for i in range(config.num_gpus):
            with tf.device('/gpu:{}'.format(i)):
                with tf.variable_scope('model',reuse=reuse_vars):
                    model=cnn_lstm_model.Model(split_features[i],split_seq_len[i],config,is_training=True)
                    logits=model.logits
        ...
        with tf.device('/cpu:0'):
            with tf.variable_scope('model',reuse=True):
                ...
                val_model = cnn_lstm_model.Model(features, sequence_length, config,is_training=False)
                print('done.\n')
