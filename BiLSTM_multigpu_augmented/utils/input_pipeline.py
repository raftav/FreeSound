...
def get_training_files(epoch):
    random.seed(epoch)
    file_list=[]
    max_examples_per_class=1020
    #max_examples_per_class=1

    for label_class in range(0,41):
        class_files=sorted(glob.glob('/home/rtavaron/KaggleFreesound/data/features_train_augmented/sequence_class{:d}_*'.format(label_class)))
...
def get_test_files():
    files=sorted(glob.glob('/home/rtavaron/KaggleFreesound/data/features_test/*.tfrecords'))
    return files
