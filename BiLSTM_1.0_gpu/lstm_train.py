    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True,
            ...
            for epoch in range(1,config.num_epochs):
                ...
                while True:
                    try:
                        _, C, train_pred, train_label , lr = sess.run([model.optimize, model.cost,
                                                                  model.prediction,
                                                                  model.labels,
                                                                  model.learning_rate],feed_dict={keep_prob:config.keep_prob})


                        epoch_cost += C

                        if (step % 10 == 0 or step == 1):
                            print("step[{:7d}] cost[{:2.5f}] lr[{}] time[{}]".format(step, C,lr,time.time()-partial_time))
                            partial_time=time.time()
