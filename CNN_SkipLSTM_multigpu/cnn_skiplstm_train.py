...
import cnn_skiplstm_model_independent
    ...
    with tf.Graph().as_default():
        ...
        for i in range(config.num_gpus):
            with tf.device('/gpu:{}'.format(i)):
                with tf.variable_scope('model',reuse=reuse_vars):
                    model=cnn_skiplstm_model_independent.Model(split_features[i],split_seq_len[i],config,is_training=True)
                    logits=model.logits
                    all_updates=model.all_states

                    with tf.name_scope('cost'):
                        cost1 = tf.reduce_mean(
                            tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits, labels=split_labels[i]))
                        cost2 = config.lambda_upd * all_updates
                        cost=cost1+cost2
        ...
        with tf.device('/cpu:0'):
            with tf.variable_scope('model',reuse=True):
                ...
                val_model = cnn_skiplstm_model_independent.Model(features, sequence_length, config,is_training=False)
                print('done.\n')
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True,
            ...
            print('## lambda : ',config.lambda_upd)
            trainingLogFile.write('## lambda : {} \n'.format(config.lambda_upd))

            print('## num gpus : ',config.num_gpus)
            trainingLogFile.write('## num_gpus : {} \n'.format(config.num_gpus))
            ...
            for epoch in range(1,config.num_epochs):
                ...
                if ((epoch % plot_every_epoch) == 0):
                    ...
                    while True:
                        try:
                            example_accuracy, val_label, val_prediction , updates_fw, updates_bw = sess.run([accuracy,
                                                                                    labels,
                                                                                    prediction,
                                                                                    val_model.updates_fw,
                                                                                    val_model.updates_bw])
                            val_accuracy += example_accuracy
                            updates_fw_rate += np.sum(updates_fw) / updates_fw.shape[1]
                            updates_bw_rate += np.sum(updates_bw) / updates_bw.shape[1]

                            '''
                            print('label[{}] prediction[{}] accuracy[{}] length[{}] updates_fw[{}] updates_bw[{}]'.format(val_label,
                                                                                     val_prediction,
                                                                                     example_accuracy,
                                                                                     updates_fw.shape[1],
                                                                                     np.sum(updates_fw),
                    ...
                    updates_fw_rate /= num_examples_val
                    updates_bw_rate /= num_examples_val

                    print('Validation accuracy : {} '.format(val_accuracy))
                    print('Forward  updates rate : {}'.format(updates_fw_rate))
                    print('Backward updates rate : {}'.format(updates_bw_rate))
                    print('Validation time : {}'.format(time.time()-ValidationStartTime))

                    '''
                    accuracy_list.append(val_accuracy)
                    if len(accuracy_list) > 5:
                        if val_accuracy <= (max(accuracy_list[-5]) + 0.0001):
                    ...
                    outstring='{}\t{}\t{}\n'.format(epoch,epoch_cost,val_accuracy)
