...
import lstm_model_test as lstm_model
	...
	with tf.Graph().as_default():
		...
		with tf.device('/cpu:0'):
			...
			dataset = dataset.padded_batch(batch_size, padded_shapes=([],
																	  [None, 41])).filter(lambda x, y: tf.equal(tf.shape(x)[0], tf.cast(batch_size,tf.int32)))
		...
		with tf.Session(config=tf.ConfigProto(allow_soft_placement=True,
			...
			while True:
				try:
					...
					filename=filenames_test[val_example_number].replace('/home/rtavaron/KaggleFreesound/data/test_feat/','').replace('.tfrecords','.wav')
